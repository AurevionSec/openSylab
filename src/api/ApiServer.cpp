...
bool parseJsonValue(const std::string &input, size_t &pos, std::string &out) {
  ...
  if (input[pos] == '"') {
    return parseJsonString(input, pos, out);
  }
...
bool parseJsonObject(const std::string &input,
  ...
  while (pos < input.size()) {
    skipWhitespace(input, pos);
    std::string key;
    if (!parseJsonString(input, pos, key)) {
    ...
    ++pos;
    std::string value;
    if (!parseJsonValue(input, pos, value)) {
...
ApiResponse ApiRouter::handleRequest(const ApiRequest &request) {
  ...
  if (path == "/api/v1/samples") {
    ...
    if (fromIt != query.end()) {
      try {
        filter.fromDate = static_cast<std::time_t>(std::stoll(fromIt->second));
      } catch (...) {
    ...
    if (toIt != query.end()) {
      try {
        filter.toDate = static_cast<std::time_t>(std::stoll(toIt->second));
      } catch (...) {
    ...
    core::AuditEntry entry(core::AuditEntry::ActionType::UPDATE,
                           core::AuditEntry::EntityType::SYSTEM, "api_samples",
                           actor, "API READ /samples");
  ...
  if (path.rfind("/api/v1/samples/", 0) == 0) {
    const std::string sampleId = path.substr(std::string("/api/v1/samples/").size());
  ...
  if (path == "/api/v1/orders") {
    ...
    core::AuditEntry entry(core::AuditEntry::ActionType::UPDATE,
                           core::AuditEntry::EntityType::SYSTEM, "api_orders",
                           actor, "API READ /orders");
    (void)database_->logAudit(entry);
  ...
  if (path.rfind("/api/v1/orders/", 0) == 0) {
    const std::string orderId = path.substr(std::string("/api/v1/orders/").size());
  ...
  if (path == "/api/v1/results") {
    ...
    if (orderIt != query.end()) {
      try {
        results = database_->getTestResultsByOrderId(std::stoi(orderIt->second));
      } catch (...) {
        return makeError(400, "validation_error", "Invalid order_id",
                         "Provide numeric order_id.");
      }
    ...
    core::AuditEntry entry(core::AuditEntry::ActionType::UPDATE,
                           core::AuditEntry::EntityType::SYSTEM, "api_results",
                           actor, "API READ /results");
    (void)database_->logAudit(entry);
  ...
  if (path.rfind("/api/v1/results/", 0) == 0) {
    const std::string resultId = path.substr(std::string("/api/v1/results/").size());
